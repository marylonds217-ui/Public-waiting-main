...
git push origin mainconst { Client, GatewayIntentBits, EmbedBuilder, ChannelType, PermissionsBitField, SlashCommandBuilder, REST, Routes, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle } = require('discord.js');
...
const commands = [
    ...
    new SlashCommandBuilder()
        .setName('launch')
        .setDescription('تشغيل تطبيق Vina')
        .setContexts([0, 1, 2])
                    ...
                    try {
                        ...
                        if (owner && owner.user) {
                            const broadcastEmbed = new EmbedBuilder()
                                .setColor(0xFFFFFF)
                                .setTitle('📢 إشعار من مالك بوت Vina')
    ...
    if (command === 'dm') {
        ...
        try {
            ...
            const dmEmbed = new EmbedBuilder()
                .setColor(0xFFFFFF)
                .setTitle('رساله من مطور بوت  Vina :>')
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (lockedServers.includes(guild.id)) {
        return interaction.reply({ 
            content: '❌ **يجب تجديد الاشتراك :<**\n\nموقع تجديد الاشتراك: [ https://vina-bot.pages.dev/ ]',
            ephemeral: true 
        });
    }
    ...
    if (commandName === 'launch') {
        const launchEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: '🎮 **Vina Games**', value: 'تشغيل الألعاب المصغرة', inline: true },
                ...
                { name: '🤖 **Vina AI**', value: 'المساعد الذكي', inline: true },
                { name: '🎨 **Vina Customizer**', value: 'تخصيص البوت', inline: true }
            )
            .setFooter({ text: 'Vina App Launcher v1.0' })
            .setTimestamp();
        ...
        const row2 = new ActionRowBuilder()
            .addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setStyle(ButtonStyle.Success),
                new ButtonBuilder()
                    .setURL('https://vina-bot.pages.dev')
                    .setLabel('🌐 Website')
    ...
    try {
        ...
        if (owner) {
            const welcomeEmbed = new EmbedBuilder()
                ...
                .setFooter({ 
                    text: `Vina Support Bot | ${new Date().toLocaleDateString('ar-SA')}`, 
                    iconURL: 'https://cdn.discordapp.com/attachments/1449057765397106830/1459265170584109067/8ed9b44c0b845fd2d1b092949bc83411.jpg?ex=69898a58&is=698838d8&hm=e64f57cb8ba535d347da7ea478c1400ff5da0d71018f631fc176bc96d51b9889&' 
                })
                .setTimestamp();
    ...
    for (const admin of admins.values()) {
        try {
            if (admin.id !== guild.ownerId) {
                const helpEmbed = new EmbedBuilder()
                    .setColor(0x3498db)
                    .setTitle('👋 مرحباً بك في بوت الدعم الصوتي Vina!')
    ...
    try {
        ...
        if (textChannel) {
            const publicEmbed = new EmbedBuilder()
                ...
                .setDescription(`**تمت إضافة بوت Vina بنجاح إلى ${guild.name}`)
                .addFields({
                    name: '📝 الخطوات الأولية',
                    value: ' استخدم `/help` لعرض الأوامر'
                })
                .setFooter({ text: 'Vina Support Bot | نظام دعم صوتي متكامل' })
                .setTimestamp();
